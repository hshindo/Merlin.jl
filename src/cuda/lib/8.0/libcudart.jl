...
function cudaDeviceGetP2PAttribute(value,attr,srcDevice,dstDevice)
    checkerror(ccall((:cudaDeviceGetP2PAttribute,libcudart),cudaError_t,(Ptr{Cint},cudaDeviceP2PAttr,Cint,Cint),value,attr,srcDevice,dstDevice))
end
...
function cudaMemPrefetchAsync(devPtr,count,dstDevice,stream)
    checkerror(ccall((:cudaMemPrefetchAsync,libcudart),cudaError_t,(Ptr{Void},Csize_t,Cint,cudaStream_t),devPtr,count,dstDevice,stream))
end

function cudaMemAdvise(devPtr,count,advice,device)
    checkerror(ccall((:cudaMemAdvise,libcudart),cudaError_t,(Ptr{Void},Csize_t,cudaMemoryAdvise,Cint),devPtr,count,advice,device))
end

function cudaMemRangeGetAttribute(data,dataSize,attribute,devPtr,count)
    checkerror(ccall((:cudaMemRangeGetAttribute,libcudart),cudaError_t,(Ptr{Void},Csize_t,cudaMemRangeAttribute,Ptr{Void},Csize_t),data,dataSize,attribute,devPtr,count))
end

function cudaMemRangeGetAttributes(data,dataSizes,attributes,numAttributes,devPtr,count)
    checkerror(ccall((:cudaMemRangeGetAttributes,libcudart),cudaError_t,(Ptr{Ptr{Void}},Ptr{Csize_t},Ptr{cudaMemRangeAttribute},Csize_t,Ptr{Void},Csize_t),data,dataSizes,attributes,numAttributes,devPtr,count))
end
