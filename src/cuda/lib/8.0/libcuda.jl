# Julia wrapper for header: /home/shindo/local-lemon/cuda-8.0/include/cuda.h
...
function cuMemPrefetchAsync(devPtr,count,dstDevice,hStream)
    check_curesult(ccall((:cuMemPrefetchAsync,libcuda),CUresult,(CUdeviceptr,Csize_t,CUdevice,CUstream),devPtr,count,dstDevice,hStream))
end

function cuMemAdvise(devPtr,count,advice,device)
    check_curesult(ccall((:cuMemAdvise,libcuda),CUresult,(CUdeviceptr,Csize_t,CUmem_advise,CUdevice),devPtr,count,advice,device))
end

function cuMemRangeGetAttribute(data,dataSize,attribute,devPtr,count)
    check_curesult(ccall((:cuMemRangeGetAttribute,libcuda),CUresult,(Ptr{Void},Csize_t,CUmem_range_attribute,CUdeviceptr,Csize_t),data,dataSize,attribute,devPtr,count))
end

function cuMemRangeGetAttributes(data,dataSizes,attributes,numAttributes,devPtr,count)
    check_curesult(ccall((:cuMemRangeGetAttributes,libcuda),CUresult,(Ptr{Ptr{Void}},Ptr{Csize_t},Ptr{CUmem_range_attribute},Csize_t,CUdeviceptr,Csize_t),data,dataSizes,attributes,numAttributes,devPtr,count))
end
...
function cuStreamWaitValue32(stream,addr,value,flags)
    check_curesult(ccall((:cuStreamWaitValue32,libcuda),CUresult,(CUstream,CUdeviceptr,cuuint32_t,UInt32),stream,addr,value,flags))
end

function cuStreamWriteValue32(stream,addr,value,flags)
    check_curesult(ccall((:cuStreamWriteValue32,libcuda),CUresult,(CUstream,CUdeviceptr,cuuint32_t,UInt32),stream,addr,value,flags))
end

function cuStreamBatchMemOp(stream,count,paramArray,flags)
    check_curesult(ccall((:cuStreamBatchMemOp,libcuda),CUresult,(CUstream,UInt32,Ptr{CUstreamBatchMemOpParams},UInt32),stream,count,paramArray,flags))
end
...
function cuTexRefSetBorderColor(hTexRef,pBorderColor)
    check_curesult(ccall((:cuTexRefSetBorderColor,libcuda),CUresult,(CUtexref,Ptr{Cfloat}),hTexRef,pBorderColor))
end
...
function cuTexRefGetBorderColor(pBorderColor,hTexRef)
    check_curesult(ccall((:cuTexRefGetBorderColor,libcuda),CUresult,(Ptr{Cfloat},CUtexref),pBorderColor,hTexRef))
end
...
function cuDeviceGetP2PAttribute(value,attrib,srcDevice,dstDevice)
    check_curesult(ccall((:cuDeviceGetP2PAttribute,libcuda),CUresult,(Ptr{Cint},CUdevice_P2PAttribute,CUdevice,CUdevice),value,attrib,srcDevice,dstDevice))
end
