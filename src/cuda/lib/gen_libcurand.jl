...
headers = ["/usr/local/cuda/include/curand.h"]
...
function rewriter(ex::Expr)
    ...
    if rettype == :curandStatus_t
      fname = decl.args[1]
      if !in(fname, skip_error_check)
        body.args[1] = Expr(:call, :check_curandstatus, deepcopy(ccallexpr))
      end
    end
    ex
end
...
context = wrap_c.init(output_file = "libcurand.jl",
                      common_file = "libcurand_types.jl",
                      header_library = x->"libcurand",
                      headers = headers,
                      clang_includes = includes,
                      clang_diagnostics = true,
                      header_wrapped=(x,y)->contains(y,"curand"),
                      rewriter = rewriter)
