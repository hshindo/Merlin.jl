...
function devicename(dev::Int)
    ...
    @apicall :cuDeviceGetName (Ptr{Cchar},Cint,CuDevice_t) buf buflen dev
    buf[end] = 0
...
function totalmem(dev::Int)
    ref = Ref{Csize_t}()
    @apicall :cuDeviceTotalMem (Ptr{Csize_t},CuDevice_t) ref dev
    Int(ref[])
end
...
function attribute(dev::Int, code::CUdevice_attribute)
    ref = Ref{Cint}()
    @apicall :cuDeviceGetAttribute (Ptr{Cint},Cint,CuDevice_t) ref code dev
    Int(ref[])
end
