...
"""
function softmax(x::Layer, dim::Int)
  Softmax(dim, x, softmax(x.y,dim), ∇softmax!, nothing)
end
...
"""
function logsoftmax(x::Layer, dim::Int)
  Softmax(dim, x, logsoftmax(x.y,dim), ∇logsoftmax!, nothing)
end
...
backward!(l::Softmax) = hasgrad(l.x) && l.df(l.x.gy, l.y, l.gy, l.dim)
