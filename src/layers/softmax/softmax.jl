...
"""
function softmax(x::Var, dim::Int, mode::Symbol)
  Softmax(softmax(x.data,dim), nothing, [x], dim, ∇softmax!)
end
softmax(x::GraphNode, dim) = GraphNode(softmax, x, dim)

function logsoftmax(x::Var, dim)
  LogSoftmax(logsoftmax(x.data,dim), nothing, [x], dim)
end
logsoftmax(x::GraphNode, dim) = GraphNode(logsoftmax, x, dim)

function backward!(v::Softmax)
  hasgrad(v[1]) || continue
