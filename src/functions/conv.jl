...
```
"""
function Conv{T,N}(::Type{T}, window::Window{N}, channels::Tuple)
    w = rand(T, window.dims..., channels...)
    w .*= 0.002
    w .-= 0.001
...
function (f::Conv)(x::Var)
    ...
    y = conv(f.w.data, f.window, x.data)
    df(gy) = ()
    Var(y, f, (x,), df)
end

function conv{T}(w::Array{T,4}, win::Window{2}, x::Array{T,4})
    ...
    for i = 1:size(work,3)
        ...
        ccall(h, Void, (Ptr{T},Ptr{T},Cint,Cint,Cint,Cint,Cint,Cint,Cint,Cint,Cint),
            ...
            size(w,1), size(w,2), pad(win,1), pad(win,2), stride(win,1), stride(win,2))
        println(work_i)
        BLAS.gemm!('N', 'N', T(1), work_i, w, T(0), view(y,:,:,i))
    end
    y = reshape(y, outdims[1], outdims[2], size(y,2), size(y,3))
    #df(gy::Array) = ∇conv!(f, gy, work, vx.data, vx.grad)
    #println(work)
    y
end
