...
function lrn{T}(x::Array{T,4}, n::Int, k::Float64, alpha::Float64, beta::Float64)
    ...
    for i=0:size(x,4)-1
        for j=1:batoffs
            padsqx[j + padpreoffs + i*padbatoffs] = (Float64(x[j + i*batoffs]))^2 * alphaovern
        end
...
function ∇lrn!{T}(x::Array{T,4}, gx::Array{T,4}, y::Array{T,4}, gy::Array{T,4},
    scale::Array{Float64,4}, n::Int, alpha::Float64, beta::Float64)
    ...
    for i=0:size(x,4)-1
        for j=1:batoffs
            padsqx[j + padpreoffs + i*padbatoffs] = -2 * alphaovern * beta * (
    ...
    for i=1:length(x)
        gx[i] += gy[i] * T(scale[i]) + x[i] * T(gscale[i])
    end
end
